...
def listar_sets_imagenes_producto(nombre):
    ...
    for orig in originales:
        ...
        if not variantes:
            # Si no hay variantes generadas, usar la original misma
            variantes = [orig]
...
def cmd_generar_imagenes(args):
    ...
    for producto in productos:
        ...
        # Contar imágenes originales (no variantes)
...
def cmd_asignar(args):
        ...
        for archivo_cuenta in CUENTAS_DIR.iterdir():
            if archivo_cuenta.suffix == ".json":
                ...
                for pub in otra_cuenta.get("publicaciones", []):
                    prod = pub["producto"]
                    # Imágenes: guardar el set completo como tupla para comparar
    ...
    for producto in productos:
        ...
        usados = sets_globales_usados.get(producto, set())

        # Filtrar sets que ya fueron usados globalmente
        disponibles = [s for s in todos_sets if tuple(s) not in usados]

        if not disponibles:
            if todos_sets:
                print(f"  ⚠️  {producto}: sin sets de imágenes disponibles (todos quemados)")
            sin_recursos += 1
            continue
